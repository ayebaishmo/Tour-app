...
const Home = ({ navigation }) => {
  ...
  const fetchNearByHotels = async (lat, lon) => {
    ...
    await axios
      .get(
        `${baseUrl}/nearbysearch/json?location=${lat},${lon}&radius=1000&type=hotel&keyword=lodging,spa&key=${apiKey}`
      )
      .then((res) => {
          ...
          <ScrollView horizontal={true}>
            <View
              ...
              <HomeCard
                name="Boda"
                icon="ios-bicycle"
                onPressed={() => navigation.navigate("ComingSoonScreen")}
              />
              <HomeCard
                name="Taxi"
                icon="car-sport"
          ...
          {loading ? (
            ...
            <ScrollView horizontal={true}>
              {places?.map((hotel, index) => (
                <TouchableOpacity key={hotel.place_id} onPress={() => {}}>
