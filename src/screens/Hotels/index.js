...
const Hotels = () => {
  ...
  const fetchNearByHotels = async (lat, lon) => {
    ...
    await axios
      .get(
        `${baseUrl}/nearbysearch/json?location=${lat},${lon}&radius=2000&type=hotel&keyword=lodging,spa&key=${apiKey}`
      )
      .then((res) => {
  ...
  const fetchPlaceDetails = async (placeId) => {
    ...
    await axios
      .get(
        `${baseUrl}/details/json?place_id=${placeId}&fields=address_component,business_status,formatted_address,geometry,icon,name,photo,place_id,plus_code,type,url,utc_offset,vicinity,formatted_phone_number,international_phone_number,opening_hours,website,price_level,rating,review,user_ratings_total&key=${apiKey}`
      )
      .then((res) => {
                  ...
                  <Card
                    ...
                    <View>
                      {hotel.photos ? (
                        <Image
                          source={{
                            uri: `${baseUrl}/photo?maxwidth=400&photoreference=${hotel.photos[0].photo_reference}&key=${apiKey}`,
                          }}
          ...
          <BottomSheetScrollView style={{ paddingBottom: 20 }}>
            <View style={{ marginBottom: 18 }}>
              {selectedHotel.photos ? (
                <Image
                  source={{
                    uri: `${baseUrl}/photo?maxwidth=400&photoreference=${selectedHotel.photos[0].photo_reference}&key=${apiKey}`,
                  }}
